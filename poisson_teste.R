...
ggplot2::ggplot(data = df_dpois_chamadas, mapping = aes(x = n_de_chamadas_ph, y = prob)) +
  geom_line() +
  geom_text(aes(label = round(prob,4), y = prob + 0.01),
            position = position_dodge(0.9),
            size = 3,
            vjust = 0) +
  labs(title = "Distribui??o de Poisson",
...
ggplot2::ggplot(transform(data.frame(x=c(0:5)), y=dpois(x, 5)), aes(x, y)) + 
...
ggplot2::ggplot(df_poisson, aes(x = factor(events), y = density)) +
  geom_col() +
  geom_text(
